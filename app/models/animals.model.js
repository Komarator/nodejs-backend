...
Animal.create = (newAnimal, result) => {
    sql.query("INSERT INTO users SET ?", newAnimal, (err, res) => {
        if (err) {
...
Animal.updateById = (id, animal, result) => {
    sql.query(
        "UPDATE animals SET animal_breed = ?, animal_name = ?, animal_age = ? , date_of_registration = ?, release_date = ?, date_of_death = ? WHERE id = ?",
        [animal.animal_breed, animal.animal_name, animal.animal_age,animal.date_of_registration,animal.release_date,animal.date_of_death, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
